...
from lyzr_service import router as lyzr_router
...
app = FastAPI(
    title="Kairo - Compliance Knowledge Graph API",
    description="Backend Graph RAG API for Kairo Enterprise Compliance",
    version="2.1.0",
)
...
app.include_router(lyzr_router)
...
@app.get("/metrics")
def evaluation_metrics(current_user: User = Depends(get_current_user)):
    """Serve the latest evaluation report (the four judged deliverables:
    retrieval precision, entity-extraction F1, hallucination containment,
    citation traceability). Generated by `python -m eval.evaluate`.

    Read-only and defensive: returns available=False rather than erroring when
    no report has been generated yet, so the UI can render an empty state."""
    report_path = os.path.join(os.path.dirname(__file__), "eval", "last_report.json")
    if not os.path.exists(report_path):
        return {"available": False, "summary": None,
                "detail": "No evaluation report yet. Run: python -m eval.evaluate"}
    try:
        with open(report_path, encoding="utf-8") as f:
            report = json.load(f)
        return {"available": True, "generated_at": report.get("generated_at"),
                "summary": report.get("summary"), "counts": report.get("counts")}
    except Exception as e:  # noqa: BLE001
        return {"available": False, "summary": None, "detail": f"Could not read report: {e}"}
